...
def ieee_api(query):
    ...
    while True:
        ...
        url = "http://ieeexploreapi.ieee.org/api/v1/search/articles?apikey="+apikey + \
            "&format=json&max_records=" + \
            str(max_records)+"&start_record="+str(start_record)+"&"+query
    ...
    df.rename(columns={'authors.authors': 'author',
